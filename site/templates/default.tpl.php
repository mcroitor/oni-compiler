<!DOCTYPE html>
<html lang="en">
    <head>
        <title>sample</title>
        <meta encoding=UTF-8" />
        <!-- additional_meta -->
        <link rel="stylesheet" href="./styles/normalize.css" />
        <link rel="stylesheet" href="./styles/skeleton.css" />
        <link rel="stylesheet" href="./styles/defines.css" />
        <link rel="stylesheet" href="./scripts/smde/simplemde.min.css" />
    </head>
    <body>
        <div id="page-container" class="container">
            <header id="page-header" class="row"><!-- page_header --></header>
            <menu id="page-primary-menu" class="row"><!-- page_primary_menu --></menu>
            <div class="row">
                <section id="page-content" class="color-1 <!-- main -->"><!-- page_content --></section>
                <section id="page-aside-content" class="color-1 <!-- aside -->"><!-- page_aside --></section>
            </div>
            <footer id="page-footer" class="row"><!-- page_footer --></footer>
        </div>
    </body>
</html>
