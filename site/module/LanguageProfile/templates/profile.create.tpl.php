<form method="post" action="/?q=profile/create">
    <fieldset>
        <table class='u-full-width'>
            <caption>profile definition</caption>
            <tr>
                <td class="three">
                    <label for="profile-name">name</label>
                </td>
                <td class="nine">
                    <input type="text" name="profile-name" id="profile-name" class='u-full-width' />
                </td>
            </tr>
            <tr>
                <td>
                    <label for="extensions">extensions</label>
                </td>
                <td>
                    <input type="text" name="extensions" id="extensions" class='u-full-width' />
                </td>
            </tr>
            <tr>
                <td>
                    <label for="compiler-path">compiler path</label>
                </td>
                <td>
                    <input type="text" name="compiler-path" id="compiler-path" class='u-full-width' />
                </td>
            </tr>
            <tr>
                <td>
                    <label for="compiler-output">compiler output</label>
                </td>
                <td>
                    <input type="text" name="compiler-output" id="compiler-output" class='u-full-width' />
                </td>
            </tr>
            <tr>
                <td>
                    <label for="compile-command">compile command</label>
                </td>
                <td>
                    <input type="text" name="compile-command" id="compile-command" value="<!-- compile-command -->" class='u-full-width' />
                </td>
            </tr>
            <tr>
                <td>
                    <label for="interpreter-path">interpreter path</label>
                </td>
                <td>
                    <input type="text" name="interpreter-path" id="interpreter-path" class='u-full-width' />
                </td>
            </tr>
            <tr>
                <td>
                    <label for="execute-command">execute command</label>
                </td>
                <td>
                    <input type="text" name="execute-command" id="execute-command" value="<!-- execute-command -->" class='u-full-width' />
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <input type="submit" value="create" />
                </td>
            </tr>
        </table>
    </fieldset>
</form>